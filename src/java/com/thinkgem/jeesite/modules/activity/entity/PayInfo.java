...
import com.thinkgem.jeesite.modules.activity.utils.DateUtil;
...
public class PayInfo implements Serializable{
	...
	public PayInfo(SecPay secPay,HttpServletRequest request) {
		...
		this.setNotify_url(Global.getConfig("URL_NOTIFY")); // 异步接收微信支付结果通知的回调地址，通知url必须为外网可访问的url，不能携带参数。
		this.setTrade_type("JSAPI"); // JSAPI--JSAPI支付（或小程序支付）、NATIVE--Native支付、APP--app支付，MWEB--H5支付，不同trade_type决定了调起支付的方式，请根据支付产品正确上传      MICROPAY--付款码支付，付款码支付有单独的支付接口，所以接口不需要上传，该字段在对账单中会出现
